"""Real-backend capture for the engineer film. Runs INSIDE the api container
(`docker exec -i -w /app newton2-api-1 python - <stage> [args]` with this file on stdin, and
the deck at /tmp/deck.pptx). Nothing here writes Newton's words: it uploads a real deck as
...
STATE = "/tmp/eng_state.json"
DECK = "/tmp/deck.pptx"
DECK_NAME = "ME3310_Lecture9_Beam_Bending.pptx"
...
async def stage_setup():
    ...
    async with httpx.AsyncClient(timeout=60) as c:
        ...
        r = await c.post(f"{API}/documents/upload", headers=h, files={
            "file": (DECK_NAME, raw, "application/vnd.openxmlformats-officedocument.presentationml.presentation")})
        r.raise_for_status()
        doc = r.json()
        s["doc"] = doc
