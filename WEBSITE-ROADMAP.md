...
- [x] **Product changes made because of this film** (ROADMAP.md, Phase 16.1). First pass: the highlight toolbar in Write mode, and markdown in Define/Explain cards. Product owner, on seeing Write mode print a raw ` ```newton-note ` fence with JSON: "combine the WRITE and preview modes - it doesn't really need two separate modes ... I want you to do it in the actual product too, not just the demo." So the Notepad is now ONE live editor (`NoteEditor`): rendered until clicked, raw only while editing, Newton's answers always cards. The film was re-recorded on the real component.
