...
## Phase 3.18 — "Request Changes" now visibly acknowledges the click, and focuses the composer
Product owner, verbatim: "once a user clicks 'Request Changes' for a plan mode in research, there is really no acknowledgement in the UI/UX that they are requesting a change. Maybe add a tag in the chat, or open a modal below to fill it out, figure out a clean way, make sure that the textbar is already open (ready to type, no need to click on it - once you request changes) ... i feel like this is good UI/UX polish."
- [x] **A real banner, not a silent focus**: `PaperPlanCard`'s "Request Changes" and `ArtifactPlanCard`'s "Change it" now call `onRequestChanges` with the plan's own title, threaded through `CodeBlock`/`MessageContent`/`ChatPane` to `App.tsx`'s `handleFocusComposer`, which both focuses the composer (already did) AND sets a new `requestingChangesFor` state. `Composer.tsx` shows it as `Requesting changes to "<title>"` — the same accent-colored banner style as the existing "Editing message" indicator (Cancel button included), so this reads as an established app convention rather than a one-off. Clears the moment the student actually sends their tweak (or switches sessions), same lifecycle as `editing`.
- [x] **The composer was already being focused** on click (`Composer.tsx`'s `focus()` ref handle) — the missing piece was purely the visible acknowledgement, not the focus itself.
- [x] **Tests**: `PaperPlanCard.test.tsx`/`ArtifactPlanCard.test.tsx`/`ChatPane.test.tsx` updated to assert the plan's title is passed through; new `Composer.test.tsx` suite (banner text, Cancel leaves any typed draft untouched — unlike `editing`'s Cancel, this never owned the draft — Send passes through normally, editing takes precedence if both were somehow set); new `App.test.tsx` suite driving the real end-to-end flow (click -> banner appears + composer focused -> Cancel dismisses -> sending clears it). 757 desktop tests, `tsc`, `vite build` clean.
- [x] **Research film updated to demonstrate it**: `ScriptedComposer` (the film's frame-driven Composer stand-in) gained the same `requestingChangesFor` prop, rendering the identical real banner classnames; `ResearchFilm.tsx` shows it from the real "Request Changes" click through to the revised text being sent, naming the real captured plan's actual title (parsed straight out of the captured `paper-plan` JSON, not hand-typed). Re-rendered, 0 flicker outliers, website copy re-delivered, audio version re-delivered to the owner's Desktop.

## Phase 3.19 — A document card sits BELOW the message text, not above it
Product owner, verbatim: "Can you make the attached documents created appear BELOW the message output, not above (so a user doesn't have to scroll up to access the documents)."
- [x] `MessageBubble.tsx`'s `attached-documents` block (the real, clickable document card from Phase 3.17 — or a student's own composer attach, same code path either way) moved to render AFTER `MessageContent`, not before it: the message text is what a student reads first, and the document it's about (or that Newton just produced) is the very next thing, with no scrolling back up past a long reply to reach it. `App.css`'s `margin-bottom` on that block became `margin-top` to match. New `MessageBubble.test.tsx` test asserts the DOM order directly (`compareDocumentPosition`). 758 desktop tests, `tsc` clean.
- [x] The research film needed no code changes at all to pick this up — it renders the real `MessageBubble` component directly. Re-rendered as the one change requested; 0 flicker outliers; audio version re-delivered to the owner's Desktop, website copy re-delivered.
