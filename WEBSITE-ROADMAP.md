...
- [x] **Product changes made because of this film:** the arXiv style, several source documents and the author's name (ROADMAP.md, Phase 33).
- [x] **Rendering notes:** the app previews a PDF in an `<iframe>`, which a headless renderer cannot draw, so the film hides the frame (`promo.css`) and lays the real page images over the pane; the viewer window is an OS-style frame drawn by the film. Every wall-clock timeout in film code counts iterations instead (`Date.now()` is frozen while rendering) - a poll waiting on a Date-based timeout hung the first render. Each film's `fetch` mock is gated by `src/filmId.ts`. Flicker check: 0 single-frame outliers over 2688 frames.
- [x] **Sound** (`scripts/export-cues-paper.ts` -> `make_audio.py`): key clicks, upload rises and landing chimes, the attach snap, message pops, source blips, a soft hum while the paper is written and a chime when it is done, page turns for the PDF, calm music (76 bpm, -17.2 LUFS). The site copy is silent; the audio version was delivered to the owner's Desktop (`Newton-research-paper-film-with-audio.mp4`).
- [x] **Website:** a new `ResearchFilmSection` ("From messy notes to a real paper.", `#demo-research`) under the main film, reusing `FilmPlayer` (autoplay in view, reduced-motion Play button, click to pause), playing `/demo/newton-research.mp4` (10.4 MB, no audio track) with its poster. Tests: the section's own test, and the page test now expects exactly two films. `tsc`, 60 website tests and `next build` clean.
