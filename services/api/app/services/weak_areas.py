...
async def get_weak_areas(db: AsyncSession, user_id: uuid.UUID) -> list[WeakArea]:
    ...
    weak_by_doc: dict[uuid.UUID | None, list[str]] = {}
    for card in cards:
        logs = (
            await db.execute(
        ...
        if weak_count > len(logs) / 2:
            weak_by_doc.setdefault(card.document_id, []).append(card.front)

    missed_by_doc: dict[uuid.UUID | None, list[str]] = {}
    exams = (
        await db.execute(
    ...
    for exam in exams:
        ...
        for q in questions:
            missed_by_doc.setdefault(exam.document_id, []).append(q.question)
    ...
    for key in set(weak_by_doc) | set(missed_by_doc):
        ...
        areas.append(
            WeakArea(
                label=label,
                weak_flashcards=weak_by_doc.get(key, [])[:_MAX_EXAMPLES_PER_GROUP],
                missed_questions=missed_by_doc.get(key, [])[:_MAX_EXAMPLES_PER_GROUP],
            )
        )
