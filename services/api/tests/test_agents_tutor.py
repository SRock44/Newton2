...
async def test_run_tutor_streams_directly_when_no_tool_needed(monkeypatch):
    ...
    chunks = [c async for c in tutor.run_tutor(str(uuid.uuid4()), "hi")]
...
async def test_run_tutor_executes_a_tool_call_then_answers(monkeypatch):
    ...
    chunks = [c async for c in tutor.run_tutor(str(uuid.uuid4()), "what is 6 times 7?")]
...
async def test_run_tutor_handles_a_tool_error_gracefully(monkeypatch):
    ...
    chunks = [c async for c in tutor.run_tutor(str(uuid.uuid4()), "bad expr")]

    assert "".join(chunks) == "Couldn't compute that."
...
async def test_run_tutor_stops_after_max_rounds_with_a_clear_message(monkeypatch):
    ...
    chunks = [c async for c in tutor.run_tutor(str(uuid.uuid4()), "loop forever")]
